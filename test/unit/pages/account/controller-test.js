...
describe('Login controller', function(){
  ...
  describe('login', function() {
    ...
    it('should set authFailed after failure', function(){
      http.returns(Promise.reject('failure'));
    ...
    it('should set authFailed when return code of HTTP request is not 200', function(done){
      http.returns(Promise.resolve({ status: 404 }));
    ...
    it('should set tandcNotSigned when no data returned', function(){
      http.returns(Promise.resolve({ status: 200, data: false }));
    ...
    it('should go to the page "confirm" when everything is OK', function(){
      http.returns(Promise.resolve({ status: 200, data: true }));
      let router = { go: function(arg) { console.log('go go go');} };
